...
module SBSM
  class InvalidDataError < RuntimeError
    ...
    def initialize(msg, key, value)
      super("#{msg} #{key.to_s[0..79]} #{value.to_s[0..79]}")
      @key = key
      @value = value
    end
  end
  ...
  class Validator
    ...
    def perform_validation(key, value)
      ...
      begin
        if key == :event
          ...
          StringIO.new(value)
        elsif !Object.methods.include?(key) && respond_to?(key, true)
          send(key, value)
        end
    ...
    def email(value)
      ...
      if e.instance_of?(SBSM::InvalidDataError)
        raise e
      else
        raise InvalidDataError.new(:e_invalid_email_address, :email, value)
      end
    ...
    alias_method :default_flavor, :flavor
    def language(value)
      validate_string(value)
    end
    ...
    alias_method :confirm_pass, :pass
    @@state_id_ptrn = /-?\d+/
    def state_id(value)
