...
module SBSM
  class	Session
    attr_reader :user, :active_thread, :key, :cookie_input, :persistent_cookie_name, :post_content,
      :server_port, :request_params, :request_method, :request_origin,
    ...
    reset_stats
    @@stats_ptrn = /./
    def self.show_stats ptrn = @@stats_ptrn
      if ptrn.is_a?(String)
        ptrn = /#{ptrn}/i
      end
    ...
    def process_rack(rack_request:)
      ...
      html = @mutex.synchronize do
        begin
          ...
          validator.reset_errors if validator&.respond_to?(:reset_errors)
          ...
          @active_state.touch
          cap_max_states
          process_late if respond_to?(:process_late) # needed for ODDB.org limit_request
        ensure
    ...
    def flavor
      @flavor ||= begin
        ...
        if lf_factory&.include?(user_input)
          user_input
        else
    ...
    def <=>(other)
      weighted_mtime <=> other.weighted_mtime
    end
